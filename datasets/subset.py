...
def distribute_subset(subset, weight, ordering_time, similarity_time, pred_time, args):
    ...
    size = torch.tensor([len(subset)], dtype=torch.int32, device=device)
    subset_sizes = [
        torch.zeros_like(size, device=device)
    ...
    subset_list = [
        torch.zeros(max_size, dtype=torch.int64, device=device) for _ in range(args.world_size)
    ]
    ...
    if args.weighted and args.greedy:
        weight_list = [
            torch.zeros(max_size, dtype=torch.float32, device=device)
            for _ in range(args.world_size)
        ]
        ...
        weight = (
            torch.cat(
                [
                    weight,
                    torch.zeros(max_size - len(weight), dtype=torch.float32, device=device),
                ]
            )
    ...
    reduced_times = torch.tensor([ordering_time, similarity_time, pred_time], dtype=torch.float32, device=device)
    dist.reduce(
...
class SubsetGenerator:
    ...
    def _greedy_features(
        ...
        if pred_loader is None:
            idx_subset = Subset(train_dataset, indices=idx)
